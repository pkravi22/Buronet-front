        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex justify-center items-end" onClick={onClose}>
            <div className="bg-white w-full max-w-lg h-[70vh] rounded-t-2xl flex flex-col p-4" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handlePostComment} className="flex gap-2 border-t pt-4">
                    <input type="text" value={newComment} onChange={e => setNewComment(e.target.value)} placeholder="Add a comment..." className="flex-1 bg-gray-100 rounded-full px-4 py-2 border-transparent focus:ring-2 focus:ring-cyan-500 focus:outline-none text-black" />
                    <button type="submit" className="bg-[#0096c7] hover:bg-cyan-700 text-white rounded-full p-2.5 flex items-center justify-center transition-colors">
                        <Send size={18}/>
                    </button>
                </form>
            </div>
