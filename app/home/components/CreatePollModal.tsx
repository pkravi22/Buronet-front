...
const CreatePollModal: React.FC<CreatePollModalProps> = ({ isOpen, onClose, onPostCreated }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validation: Content (poll question) is mandatory
    if (!content.trim()) {
      setError('Poll question is required.');
      return;
    }

    // Validation: Poll must have at least 2 options
    const filteredOptions = options.filter(option => option.trim() !== '');
    if (filteredOptions.length < 2) {
      setError('A poll must have at least 2 options.');
      return;
    ...
    const newPoll: CreatePollDto = {
      title: content.trim(), // Use content as the poll title
  ...
  return (
    ...
    >
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <h2 className="text-xl font-semibold text-gray-800 mb-6 pb-2 border-b px-6 pt-6 shrink-0">
          Create a Poll
        </h2>
          ...
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1">
              ...
              <textarea
                ...
                className={`mt-1 block w-full text-lg p-2 border rounded-lg resize-none focus:ring-indigo-500 focus:border-indigo-500 transition-colors ${
