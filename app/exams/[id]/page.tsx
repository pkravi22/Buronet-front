...
import { Exam, ApiResponse, ExamStage, AgeLimit, Paper } from '@/lib/types/exams';
...
// Helper to safely format a number for age, handling undefined
  ...
  useEffect(() => {
    ...
    const fetchExamDetails = async () => {
      ...
      try {
        ...
        const examResponse = await get<Exam>(`/exams/${examId}`);
        // if (examResponse.success) {
        setExam(examResponse); // Kept as per your original code
        // } else {
                        ...
                        <div className="bg-white rounded-xl shadow-sm p-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="bg-gray-50 rounded-lg p-4">
                                    ...
                                    {exam.eligibilityCriteria.ageLimit?.relaxationNotes.length > 0 && (
                                        <ul className="text-xs text-gray-600 list-disc pl-4 mt-1 space-y-1 marker:text-gray-400">
                                            {exam.eligibilityCriteria.ageLimit.relaxationNotes.map((note, index) => <li key={index} className="pl-1 italic">{note}</li>)}
                                        </ul>
                                    )}
                                </div>
                                <div className="bg-gray-50 rounded-lg p-4">
