...
const CompleteProfilePage: React.FC = () => {
  ...
  const { user, isLoading: authLoading, error: authError, refetchProfile } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (profileImageFile) {
        ...
        updateDto = {
          ...updateDto,
          profilePictureMediaId: res.profilePictureMediaId,
        };
      }

      await put('/Users/profile', updateDto);

      // Critical: update AuthContext profile state before navigating.
      await refetchProfile();

      // Send them to the app landing page.
      router.replace('/home');
      router.refresh();
    } catch (err: any) {
