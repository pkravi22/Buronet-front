...
const RegisterPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await register({ username, email: normalizedEmail, password } as RegisterData);
    
    console.log('Register result:', result);
    
    // If registration was successful, redirect to check-email page
    if (result && result.success) {
      ...
      console.log('Registration failed or no success flag');
    }
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-6"> {/* mt-8 for spacing above form */}
          ...
          <div>
            <button
              ...
              disabled={isSubmitting || !!emailError || !!passwordError || !!usernameError} // Disable button when invalid/loading
            >
              ...
              {isSubmitting ? 'Registering...' : 'Sign up'} {/* Button text changes based on loading state */}
            </button>
          </div>
        </form>
