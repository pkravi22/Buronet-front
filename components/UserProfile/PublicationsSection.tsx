...
const PublicationsSection: React.FC<PublicationsSectionProps> = ({ publications }) => {
  ...
  return (
    <UserProfileSection title="Publications" onAdd={handleAddClick}>
      {publications.length === 0 ? (
        <p className="text-gray-500 italic">No publications added yet. Click "Add New" to add one.</p>
      ) : (
        <div className="space-y-4">
          {publications.map((pub) => (
            <div key={pub.id} className="relative p-4 border rounded-lg bg-gray-50">
              ...
              {pub.publicationDate && <p className="text-gray-500 text-sm">Published: {format(new Date(pub.publicationDate), 'MMM d, yyyy')}</p>}
