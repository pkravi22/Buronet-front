// components/UserProfile/EditExperienceForm.tsx
...
const EditExperienceForm: React.FC<EditExperienceFormProps> = ({ experience, onClose }) => {
  const { addExperience, updateExperience } = useUserProfile();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const dataToSend: UpdateUserExperienceDto = Object.fromEntries(
    ...
    <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg my-8">
        <div className="flex justify-between items-center border-b p-4">
          <h2 className="text-2xl font-bold text-gray-800">{experience ? 'Edit Experience' : 'Add New Experience'}</h2>
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4">
            ...
            <div>
              ...
              <input type="text" id="organization" name="organization" value={formData.organization || ''} onChange={handleChange} className="form-input" />
            </div>
            <div>
            ...
            <div>
              <label htmlFor="endDate" className="block text-gray-700 text-sm font-bold mb-2">End Date (Leave blank if current)</label>
