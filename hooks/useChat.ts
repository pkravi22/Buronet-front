...
export const useChat = (): UseChatResult => {
  ...
  const fetchConversations = useCallback(async () => {
    ...
    try {
      ...
      //   console.log('useChat: Auto-selecting first conversation.');
  ...
  useEffect(() => {
      ...
      connection.on("ReceiveMessage", (message: MessageDto & { clientId?: string }) => {
        ...
        if (selectedConversation && message.conversationId === selectedConversation.id) {
          setMessages(prevMessages => {
            ...
            console.log("SignalR: Message received for non-selected conversation. Not updating current view.");
        }
        // Update last message/unread count in conversations list for all relevant conversations
        setConversations(prevConversations => prevConversations.map(conv =>
          conv.id === message.conversationId
            ? { ...conv, lastMessage: message, unreadCount: (conv.unreadCount || 0) + 1 } // Increment unread count
            : conv
        ));
      });
    ...
    return () => {
      if (connectionRef.current) { // Check if connectionRef.current is not null before stopping
        console.log('SignalR: Disconnecting from hub on unmount.');
        connectionRef.current.stop();
        connectionRef.current = null;
      }
    };
  }, [user, isAuthLoading, getAuthToken, selectedConversation, fetchConversations, fetchMessages]);
  ...
  }, [selectedConversation, isSendingMessage, user, connectionRef, postApi]);
